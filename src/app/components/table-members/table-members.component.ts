...
export class TableMembersComponent {
  ...
  formatDate(date?: Date | string): string {
    ...
    const hours = now.diff(start.add(years, 'year').add(months, 'month').add(weeks, 'week').add(days, 'day'), 'hour');

    if (years > 0) return `${years} year${years > 1 ? 's' : ''} ${months > 0 ? months + ' month' + (months > 1 ? 's' : '') : ''}`;
    if (months > 0) return `${months} month${months > 1 ? 's' : ''} ${weeks > 0 ? weeks + ' week' + (weeks > 1 ? 's' : '') : ''}`;
    if (weeks > 0) return `${weeks} week${weeks > 1 ? 's' : ''} ${days > 0 ? days + ' day' + (days > 1 ? 's' : '') : ''}`;
